...
function FighterTotalStrikes() {
  ...
  const trackerClicked = (corner: string, target: string) => {
    ...
    setBlueStats(
      target === "landed"
        ? { landed: blueStats.landed + 1, thrown: blueStats.thrown + 1 }
        : { ...blueStats, thrown: blueStats.thrown + 1 }
    );
  };
  return (
    <>
      <Stack spacing={1}>
        ...
        <Box>
          <Typography variant='body2'>
            Landed: {blueStats.landed} - Thrown: {blueStats.thrown} - Percent:{" "}
            {((blueStats.landed / blueStats.thrown) * 100).toFixed(2)}%
          </Typography>
        </Box>
      </Stack>
    </>
