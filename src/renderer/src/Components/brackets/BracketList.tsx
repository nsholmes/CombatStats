...
import {
  ...
  Outcome,
  Trigger,
} from "./BracketList.model";
import { getBasicData } from "./BracketListHelper";
import { createRegistry } from "./BracketRegistry";
...
function BracketList(props: BracketsListProps) {
  ...
  useEffect(() => {
    ...
    if (outcome.type === "card-reorder") {
      ...
      const entry = registry.getCard(item.bracketId.toString());
      triggerPostMoveFlash(entry.element);
      ...
      liveRegion.announce(
        `You've moved ${item.bracketClassName} from position ${
          startIndex + 1
  ...
  const moveCard = useCallback(
    ({
      ...
      setData((data) => {
        ...
        const destinationColumn = data.columnMap[finishColumnId];
        const item: CSBracket = sourceColumn.items[itemIndexInStartColumn];
          ...
          if (source.data.type === "card") {
            ...
            if (location.current.dropTargets.length === 2) {
              ...
              const indexOfTarget = destinationColumn.items.findIndex(
                (item) => item.bracketId === destinationCardRecord.data.itemId
              );
              const closestEdgeOfTarget: Edge | null = extractClosestEdge(
