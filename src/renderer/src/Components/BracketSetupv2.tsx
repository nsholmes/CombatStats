...
const BracketSetup = memo(function BracketSetup(props: BracketSetupProps) {
    ...
    if (bracket.competitors.length === 1) {
        ...
        <>
          <Box className='bout'>
            <Box
              ...
              className={`seat ${
                props.bracketEditState != "off" ? "editSeat" : "seat1"
              }`}
              onClick={(ev) => {
                competitorClicked(ev, bracket.competitors[0]);
              }}
      ...
      return (
        <Box className='bout'>
          <Box
            ...
            className={`seat ${
              props.bracketEditState != "off" ? "editSeat" : "seat3"
            }`}
            draggable={true}
            // onContextMenu={showContextMenu}
            onClick={(ev) => {
              competitorClicked(ev, bracket.competitors[1]);
            }}
          ...
          <Box
            id={`${bracket.bracketId}|${bracket.competitors[2].id.toString()}`}
            className={`seat ${
              props.bracketEditState != "off" ? "editSeat" : "seat2"
            }`}
            draggable={true}
            // onContextMenu={showContextMenu}
            onClick={(ev) => {
              competitorClicked(ev, bracket.competitors[2]);
            }}
            onDragStart={dragBracket}
      ...
      return (
        <Box className='bout'>
          ...
          <Box
            id={`${bracket.bracketId}|${bracket.competitors[3].id.toString()}`}
            className={`seat ${
              props.bracketEditState != "off" ? "editSeat" : "seat2"
            }`}
            draggable={true}
            // onContextMenu={showContextMenu}
            onClick={(ev) => {
              competitorClicked(ev, bracket.competitors[3]);
            }}
            onDragStart={dragBracket}
  ...
  const competitorClicked = (
    ev: MouseEvent<HTMLDivElement>,
    competitor: BracketCompetitor
  ) => {
    console.log("competitorClicked: ", competitor);
      ...
      case 2:
        if (selectedId) {
          document
            ...
            ?.classList.remove("selectedCompetitor");
        }
  ...
  const emptyBracketClicked = (
    ...
    switch (clickCount) {
      case 1:
        console.log(
          "SINGLE CLICK: If selectedCompetitor then MOVE competitor to the selected bracket seat"
        );
        if (props.getSelectedCompetitor) {
          document
            .getElementById(props.getSelectedCompetitor)
            ?.classList.remove("selectedCompetitor");
        }
        // Move selected compititor to this bracket.
        props.moveSelectedCompetitor(
          `${props.getSelectedCompetitor}|${bracketId}`
        );
        props.setSelectedCompetitor(null);
        break;
      case 2:
    ...
    <>
      <Box
        ...
        <Typography variant='body2'>
          <strong>Bracket Count:</strong> {brackets.length}{" "}
              ...
              {bracket.competitors.length > 0 ? (
                <Box>
                  <Box key={index} className='bracket4'>
                    ...
                    <Box className='round round2'>{setupRound2(bracket)}</Box>
